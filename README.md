...
- [Improvement Plan Priorities](#improvement-plan-priorities)
...
- IAM Roles attached to the EC2s to allow for the principle of least privilege and further secure the infrastructure.
...
- An ALB (Application Load Balancer) to distrubute the flow of traffic across multiple EC2s managed by an ASG (Auto Scaling Group)
...
3) Make IAM Roles to make sure that there is no privilege escalation and credentials will not need to be hardcoded into the infrastructure.
